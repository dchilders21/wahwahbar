    ...
    if (request.readyState === 4) {
      if (request.status === 200) {
        if (request.responseXML !== null) {
          ...
          onFailure("request", request, identifier, url);
        }
      } else {
        console.error("Error loading url: " + url);
        onFailure("request", request, identifier, url);
 ...
 */
function queryVAST(endpoint, identifier, onFetched, onError, parentAd) { // Wahwah change
  fetchXML(endpoint, identifier, function(doc) {
    try {
      new VASTAds(doc, identifier, onFetched, function(type, obj, identifier, url) 
	  {
  ...
  for (var i = 0; i < adElements.length; i++) {
    ...
    if (ad.hasData() && (!ad.hasSequence() || ad.isNumber(1))) {
      ...
      (function(ad, allowPods, that) {
        onGotFirstAd = function(ads, identifier) {
      ...
      queryVAST(uri, "", onGotFirstAd, onError, ad);
    }
  }
}
