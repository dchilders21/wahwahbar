 ...
 *   identifier.
 */
function fetchXML(fileRequestHelper, url, identifier, onSuccess, onFailure) {
	if (typeof(window.__RP_fileRequestHelper) == "undefined")
		window.__RP_fileRequestHelper = fileRequestHelper;
	window.__RP_fileRequestHelper(url, transferSuccess, transferFailed, "xml")
	function transferSuccess(data)
	{
		if (data !== null) {
			...
			onFailure("request", null, identifier, url);
		}
	}
	
	function transferFailed(errorJS, errorMsgFlash)
	{
		console.error("Error loading url: " + url);
		onFailure("request", null, identifier, url, "JS_ERROR:" + errorJS + "     FLASH_ERROR:" + errorMsgFlash);
	}
}
 ...
 * @param {string} identifier - identifier of call
 ...
 */
function queryVAST(fileRequestHelper, endpoint, identifier, onFetched, onError, parentAd) { // Wahwah change
  fetchXML(fileRequestHelper, endpoint, identifier, function(doc) {
    try {
      new VASTAds(doc, identifier, function(doc,that2)
	  {
		onFetched(doc, identifier);
	  }
	  , function(type, obj, identifier, url) 
	  {
	    console.error("VASTAds error");
		if (url == null)
			url = endpoint;
		onError(type, obj, identifier, url);
	  }, parentAd, endpoint);
    } catch(e) {
      console.error("Unknown error in queryVAST" + e.toString());
      var s = e.stack.split(/\n/);
	  ...
	  // wahwah change
      onError("parse", null, identifier, endpoint); 
    }
  }, onError);
}
...
function VMAP(server, adHandler, breakHandler) {
  ...
  fetchXML(window.__RP_fileRequestHelper, server, null, function(doc) {
    ...
    for (var i = 0; i < adbreaks.length; i++) {
      ...
      if (vast) {
        adbreak.ad = new VASTAds(vast.item(0).getElementByTagName(null, 'VAST').item(0), targetedAdHandler, null);
      } else {
        ...
        if (uri) {
          ...
          queryVAST(window.__RP_fileRequestHelper, uri.item(0).textContent.replace(/\s/g, ""), "", storeAd);
        } else {
 ...
 */
function VASTAds(root, identifier, onAdsAvailable, onError, parentAd, url) {
  this.ads = [];
  ...
  if (adElements.length == 0) {
    onError("parse", null, identifier, url);
    return
  }
  ...
  for (var i = 0; i < adElements.length; i++) {
    ...
    if (ad.hasData() && (!ad.hasSequence() || ad.isNumber(1))) {
      ...
      (function(ad, allowPods, that) {
        onGotFirstAd = function(doc, identifier) {
          ad.onLoaded(doc, allowPods);
          if (that.onAdsAvailable) {
            var oaf = that.onAdsAvailable;
            that.onAdsAvailable = null;
            oaf.call(doc,doc, identifier);
          }
        };
      })(ad, allowPods, that);
	  // WW - Modify adTagURI for Liverail
	  if (uri.indexOf("LR_PUBLISHER_ID=") != -1)
	  {
	  console.log("Modifying LR wrapper url");
	  // Todo Q: Does LR_MUTED=1 cause no fill? Not sure yet...
		console.log("Liverail detected. Doing Liverail-related replacements");
		// Switch back to LR_MUTED=0 for now to avoid any drop in impressions that would invalidate tests
		//uri = uri.replace(/LR_MUTED=0/gi, "LR_MUTED=1");
		if (customPubReportVals != "")
		{
			var tmpVals = customPubReportVals;
			tmpVals = tmpVals.replace(/_/g,"__");
			tmpVals = tmpVals.replace(/;/g,"_SEMI_");
			tmpVals = tmpVals.replace(/=/g,"_EQU_");
			uri+="&LR_TAGS="+encodeURIComponent(tmpVals);
		}
	  }
	  console.log("Calling wrapper VASTAdTagURI " + uri);
      queryVAST(window.__RP_fileRequestHelper, uri, identifier, onGotFirstAd, onError, ad);
    }
  }
}
...
function VASTAd(vast, root, parentAd, onAdAvailable) {
  ...
  {
		this.properties['id'] = root.getAttribute('id');
  }
  ...
  if (this.parentAd !== null) {
	...
	if (pa.impressions.length) {
      for (i = 0; i < pa.impressions.length; i++) {
        this.impressions.push(pa.impressions[i]);
      }
    }
	
	if (pa.errors.length) {
      for (i = 0; i < pa.errors.length; i++) {
        this.errors.push(pa.errors[i]);
      }
    }
    ...
    if (prop.nodeType === 1) {
      switch (prop.tagName) {
        ...
        default:
		  ...
		  if (prop.tagName = 'AdSystem' && prop.hasAttribute('version'))
		  {
			this.properties['AdSystemVersion'] = prop.getAttribute('version');
		  }
  ...
  for (i = 0; i < imps.length; i++) {
    this.impressions.push({url: imps.item(i).textContent.replace(/\s/g, "") , id: imps.item(i).getAttribute('id') });
  }
  
  // Extract Errors (TODO: We are breaking vast 2.0.1 spec by allowing multiple errors (wrapper, children) and will need to handle them later)
  var errs = inline.getElementsByTagName("Error");
  for (i = 0; i < errs.length; i++) {
    this.errors.push(errs.item(i).textContent.replace(/\s/g, ""));
  }
  ...
  for (i = 0; i < creatives.length; i++) {
    ...
    switch (creative.tagName) {
      ...
      case "CompanionAds":
        ...
        /* falls through - intentionally no break */
      case "NonLinearAds":
	...
	// Multiple extensions are possible. I'm guessing either extra stuff for VPAID ads, research study type stuff, or specific things when both player and ads are under same control
	
	var ext = inline.getElementsByTagName("Extensions"); 
	if (ext.length)
	{
		...
		{
			var extsn = ext[i];
			// {xmlNode}.innerHTML is non-standard and won't work in IE Edge for XML (surprising). Use xmlSerializer instead
			var s = "";
			// Avoid outer element (equivalent of innerHTML, not outerHTML)
			for (var j = 0; j < extsn.childNodes.length; j++)
			{
				s+=xmlSerializer.serializeToString(extsn.childNodes[j]);
			}
			this.extensions.push({type: extsn.getAttribute("type"), value: s});
		}
	}
}
...
function VASTCreative(ad, root) {
  ...
  var creativeNode = root.parentNode;
  var tmpRoot = root;
  var tag;
  if (root.tagName === "NonLinear") {
	creativeNode = root.parentNode.parentNode;
    tmpRoot = root.parentNode;
  }
  if (root.tagName === "Companion") {
	creativeNode = root.parentNode.parentNode;
    // NO root = root.parentNode HERE. TrackingEvents is in Companion;
  }
  this.tracking = new TrackingEvents(tmpRoot, ad);
  // Creative attributes
  for (var i = 0; i < creativeNode.attributes.length; i++)
  {
		var attr = creativeNode.attributes.item(i);
		this.creativeAttributes[attr.name] = attr.value;
  }
...
function VASTLinear(ad, root) {
  ...
  var param = root.getElementsByTagName("AdParameters");
  if (param.length) {
    this.adParameters = root.getElementsByTagName("AdParameters");
  }
...
function VASTCompanion(ad, root) {
  ...
  for (var i = 0; i < root.attributes.length; i++)
  {
		...
		this.attributes[attr.name] = this.attribute(attr.name);
  }
...
window.__RP_VASTVMAPReady = true;
